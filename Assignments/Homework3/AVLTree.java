...
class LUC_AVLTree {
    ...
    */
    ...
    /**
      ...
      *       scenario, it gets the in-order successor node (aka, the smallest 
      *       value node in the right subtree, this is accomplished by using 
      *       method minValueNode()). Once found, it then copies the in-order
      *       successor node's value to this node, which for all purposes deletes 
      *       the node. Finally, it needs to delete that in-order successor node 
      ...
      *
      *  In scenario 4, the routine pulls up (and deletes) the "In-order 
     ...
     * Section "4.4.2 - Double Rotation" contains the figure 4.44, which has an example for deletion but does not provide too much for this method.
     */
    private Node deleteElement(int value, Node node) {
        ...
        if (value < node.value) {
            ...
            // This is where we start to consider the scenarios detailed in the comment.

            // Case 1: Leaf Node
            // A leaf node has no left or right child node.
            ...
            // Case 2: Left Subtree
            // A Node with a left child, but no right child.
            // Remove current node from tree linking to leftChild
            if (node.leftChild != null && node.rightChild == null) {
            ...
            // A Node with a right child, but no left child.
            // Remove current node from tree linking to rightChild
            if (node.leftChild == null && node.rightChild != null) {
            ...
            // Step 1: Get inorder successor node by passing the right subtree node into minValueNode()
            // Step 2: The node we are removing gets the value of the inorder successor node
            // Step 3: Call the remove method onto the right subtree with the value copied.
            if (node.leftChild != null && node.rightChild != null) {
        ...
        // From here, we need to restore the tree's AVL properties if needed.

        // The first step is to recalculate the heights of the nodes.
        node.height = (getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild))) + 1;

        // Now we need to figure out if we have to perform a rotation by testing to see if the tree is
        // currently an AVL tree.

        // We are going to use this value to decide if a rotation is needed, and which type if needed.
    ...
    /**
      ...
      *  sub-child, zl, becomes the right sub-child of X, and right sub-child, zr, 
      *  becomes the left sub-child of Y. Node Z is returned as the new top to the 
